...
When `creator-stack:research` is invoked from this orchestrator, the YouTube Analytics MCP tools are available for data gathering. These tools provide access to:
...
1. **MANDATORY**: Invoke `creator-stack:title` to generate 3 optimized titles.
...
1. **MANDATORY**: Invoke `creator-stack:thumbnail` to generate 2 thumbnail concepts for each title. These should be concept descriptions only, not actual images yet.
...
1. **MANDATORY**: Invoke `creator-stack:hook` to generate 3 retention-optimized hooks, only for the user's selected title + thumbnail pairing.
...
1. **MANDATORY**: Invoke `creator-stack:copywriting` to create a high-level content outline.
...
- `creator-stack:title` for title generation
- `creator-stack:thumbnail` for thumbnail concepts and images
- `creator-stack:hook` for hook strategies
- `creator-stack:copywriting` for content outline
