# Newsletter-to-Video Skill Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Adopt the `temp/newsletter-to-youtube/SKILL.md` into the composable plugin system as `youtube:newsletter-to-video`.

**Architecture:** Minimal adaptation of existing skill content. Update deprecated skill references (`yt-content-strategist:youtube-video-hook` → `content-strategy:hook`), add brand compliance hook and quality checklist, place in correct plugin directory.

**Tech Stack:** Markdown (SKILL.md files), Claude Code plugin system (auto-discovered from directory structure).

---

### Task 1: Create the newsletter-to-video SKILL.md

**Files:**
- Create: `youtube/skills/newsletter-to-video/SKILL.md`
- Reference: `temp/newsletter-to-youtube/SKILL.md` (source)
- Reference: `content-strategy/skills/hook/SKILL.md` (for invocation syntax)

**Step 1: Create the skill directory and file**

Create `youtube/skills/newsletter-to-video/SKILL.md` with the following content. This is the full adapted skill — all changes from the source are noted inline:
...
Run: `ls -la youtube/skills/newsletter-to-video/SKILL.md`
Expected: File exists with reasonable size (~5KB)

Run: `wc -l youtube/skills/newsletter-to-video/SKILL.md`
Expected: Under 500 lines (framework constraint)

**Step 3: Commit**

```bash
git add youtube/skills/newsletter-to-video/SKILL.md
git commit -m "feat: add newsletter-to-video task skill to youtube plugin"
```
...
**Files:**
- Delete: `temp/newsletter-to-youtube/SKILL.md`

**Step 1: Remove the temp file and directory**

```bash
rm -rf temp/newsletter-to-youtube/
```

Check if `temp/` is now empty:
```bash
ls temp/
```

If empty, remove the temp directory too:
```bash
...
```bash
git add -A temp/
git commit -m "chore: remove temp newsletter-to-youtube after migration to youtube plugin"
```
